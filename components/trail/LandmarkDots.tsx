...
function LandmarkDot({
    ...
    // Use the same trail-space marker progress mapping that the main marker uses,
    // so landmark glow aligns with the marker's actual position on the trail.
    const markerProgressMV = useMotionValue(
        scrollProgressToMarkerProgress(
            progress,
            heroHeight,
            trailProgressHeight,
    ...
    }, [progress, heroHeight, trailProgressHeight, markerProgressMV]);

    // Landmarks are positioned in trail-space (0–1 along the path). Compare
    // marker trail progress to the landmark's trail progress so the glow
    // "breathes" as the marker approaches.
    const landmarkTrailProgress = checkpoint.locationOnTrail;

    const proximity = useTransform(
        markerProgressMV,
        [
            Math.max(0, landmarkTrailProgress - LANDMARK_OPEN_THRESHOLD),
            landmarkTrailProgress,
            Math.min(1, landmarkTrailProgress + LANDMARK_OPEN_THRESHOLD),
        ],
        [0, 1, 0],
        { clamp: true },
    );
    ...
    const fallbackX = getTrailXAtProgress(landmarkTrailProgress);
    ...
    if (path) {
        const point = path.getPointAtLength(
            clamp(landmarkTrailProgress, 0, 1) * path.getTotalLength(),
        );
        dotX = point.x;
        dotY = point.y;
    }
    ...
    return (
        <g
            ...
            onMouseEnter={(e) =>
                onHoverCheckpoint(checkpoint, { x: e.clientX, y: e.clientY })
            }
            onMouseMove={(e) =>
                onHoverCheckpoint(checkpoint, { x: e.clientX, y: e.clientY })
            }
